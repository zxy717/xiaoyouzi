...
def proposal_target_layer(rpn_rois, gt_boxes, gt_ishard, dontcare_areas, _num_classes):
    ...
    #bbox_targets_2 = bbox_targets_2.reshape(-1, _num_classes * 4)
    #bbox_inside_weights_2 = bbox_inside_weights_2.reshape(-1, _num_classes * 4)


    bbox_outside_weights = np.array(bbox_inside_weights > 0).astype(np.float32)
    #bbox_outside_weights_2 = np.array(bbox_inside_weights_2 > 0).astype(np.float32)
...
def _sample_rois(all_rois, gt_boxes, gt_ishard, dontcare_areas, fg_rois_per_image, rois_per_image, num_classes):
    ...
    #gt_assignment_2 = np.argpartition(overlaps,-2,axis=1)[:,-2]
    #index = np.stack([np.arange(len(overlaps),dtype=np.int64), gt_assignment_2])
    #max_overlaps_2 = []
    #for i in np.arange(len(overlaps)):
    #    index0 = index[:,i]
     #   max_overlaps_2 = np.append(max_overlaps_2,overlaps[index0[0],index0[1]])
    ...
    #bbox_target_data_2 = _compute_targets(
     #   rois[:, 1:5], gt_boxes[gt_assignment_2[keep_inds], :4], labels)
    ...
    #bbox_targets_2, bbox_inside_weights_2 = \
     #   _get_bbox_regression_labels(bbox_target_data_2, num_classes)
