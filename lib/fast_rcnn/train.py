...
class SolverWrapper(object):
    ...
    def train_model(self, sess, max_iters, restore=False):
        ...
        for iter in range(restore_iter, max_iters):
            ...
            #if (iter) % (cfg.TRAIN.DISPLAY) == 0:
            #    print 'iter: %d / %d, total loss: %.4f, repulsion_loss: %.4f,rpn_loss_cls: %.4f, rpn_loss_box: %.4f, loss_cls: %.4f, loss_box: %.4f, lr: %f'%\
            #            (iter, max_iters, repulsion_loss_value + rpn_loss_cls_value + rpn_loss_box_value + loss_cls_value + loss_box_value ,\
            #             repulsion_loss_value,rpn_loss_cls_value, rpn_loss_box_value,loss_cls_value, loss_box_value, lr.eval())
            #    print 'speed: {:.3f}s / iter'.format(_diff_time)
